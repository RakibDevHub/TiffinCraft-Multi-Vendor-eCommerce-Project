...
function vendorRegister($data, $files)
{
	...
	if (
		...
		header("Location: /tiffincraft/business/register?error=All+fields+are+required");
		exit();
	}

	// Validate email
	if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
		header("Location: /tiffincraft/business/register?error=Invalid+email+address");
		exit();
	}
	...
	if (oci_fetch_assoc($stid)) {
		header("Location: /tiffincraft/business/register?error=Email+already+exists");
		exit();
	}
	...
	if (!empty($image['name'])) {
		...
		if (!in_array($imageFileType, ['jpg', 'jpeg', 'png', 'gif'])) {
			header("Location: /tiffincraft/business/register?error=Invalid+image+format");
			exit();
		}

		// Move uploaded file
		if (!move_uploaded_file($image['tmp_name'], $imagePath)) {
			header("Location: /tiffincraft/business/register?error=Failed+to+upload+image");
			exit();
		}
	}
	...
	if ($result) {
		...
		header("Location: /tiffincraft/business/register?success=Request+submitted+successfully");
		exit();
	} else {
		// If insertion fails
		header("Location: /tiffincraft/business/register?error=Failed+to+submit+request");
		exit();
	}
}
